var AppSession = "../Ventas/CotizacionMilagros.aspx";
...
//            $('#MainContent_ddlFormaPago').val(i.item.FormaPago);
    ...
    $('#MainContent_btnAgregarProducto').click(function () {
        if (!F_SesionRedireccionar(AppSession)) return false;
        if (F_PermisoOpcion(CodigoMenu, 777006, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
        try {
...
//            $('#MainContent_lblTipoPrecios').text(TipoPrecios);
    ...
    $('#MainContent_btnBuscarConsulta').click(function () {
        if (!F_SesionRedireccionar(AppSession)) return false;
        if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Consultar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacio
        try {
            F_Buscar();
            return false;
        }
...
function F_ValidarPrecioGrilla(ControlID) {
    ...
    if(parseFloat(PrecioNuevo)<=0 && F_PermisoOpcion(CodigoMenu, 777016, '') === "0"   )
...
//            return false; 
    ...
    var Margen = F_Margen(Number($(lblCostoSoles).text()), PrecioMargen, 1);
    if ($('#MainContent_chkMinorista').prop('checked') == true)
    if(parseInt(Margen)<10 && F_PermisoOpcion(CodigoMenu, 777016, '') === "0"  ){
    $(lblMargenMinorista).text(Margen.toString() + '%');
...
function F_ValidarAgregar(ControlID) {
 var txtPrecio = "#"+ ControlID ;
   //empeiza
        ...
        $('#MainContent_grvConsultaArticulo .chkSi :checkbox').each(function () {
            ...
            if ($(chkSi).is(':checked')) {
                ...
                if ($(txtcantidad_grilla).val() <=0)
                    cadena = cadena + "<p></p>" + "Cantidad para el Codigo " + $(lblcodproducto_grilla).val();
...
function F_AgregarTemporal() {
    try {
        ...
        if ($('#MainContent_chkNotaPedido').is(':checked'))
            ...
            if((contararticulos + parseFloat($("#MainContent_lblNumRegistros").text())>CantidadArticulos)){
...
function F_ValidarGrabarDocumento() {
    try {
        ...
        if (isNaN($(Cuerpo + 'txtTotal').val()))
            Cadena = Cadena + '<p></p>' + 'Error en el monto';

        if ($(Cuerpo + 'txtTotal').val() == '0.00')
            Cadena = Cadena + '<p></p>' + 'No ha ingresado ningun producto';

        if (parseInt($('#MainContent_lblNumRegistros').text())>(NroItem-1)) 
...
function F_GrabarDocumento(vistaprevia) {
    try {
        ...
        var objParams = {
            ...
            Filtro_CodTransportista: 0,
            Filtro_MarcaGuia: '',
            Filtro_LicenciaGuia: '',
            Filtro_NroBultos: 0,
            Filtro_Peso: 0,
            Filtro_AcuentaNv: 0,
            Filtro_DireccionTrans: '',
...
if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Anular') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacio
    try {
        var imgID = Fila.id;
        ...
        if ($(lblEstado).text() == "ANULADO") {
            toastr.warning("LA COTIZACION SE ENCUENTRA ANULADA");
            return false;
        }

        if ($(lblEstado).text() == "CANCELADO PARCIAL") {
            toastr.warning("ESTA COTIZACION SE ENCUENTRA CANCELADA PARCIAL; PRIMERO ELIMINE LA COBRANZA Y LUEGO ANULE LA COTIZACION");
            return false;
        }

        if ($(lblEstado).text() == "CANCELADO TOTAL") {
            toastr.warning("ESTA COTIZACION SE ENCUENTRA CANCELADA TOTAL; PRIMERO ELIMINE LA COBRANZA Y LUEGO ANULE LA COTIZACION");
            return false;
        }

        if (!confirm("ESTA SEGURO DE ANULAR LA COTIZACION : " + $(lblnumero_grilla).text() + "\nDEL CLIENTE : " + $(lblcliente_grilla).text()))
            return false;
...
function F_ImprimirGuia(Fila) {
    ...
    if ($(lblCodigo).val() == '0') {
        toastr.warning("LA COTIZACION no tiene guia adjunta");
        return false;
    }
...
function ImprimirFacturaDetalle(Fila) {
    ...
    if ($(lblEstado).text() == 'ANULADO') {
        toastr.warning("La proforma se encuentra anulada");
        return false;
    }
...
function ImprimirFacturaDetalle2(Codigo) {
    var rptURL = '';
    var Params = 'width=' + (screen.width * 0.48) + ', height=' + (screen.height * 0.40) + ', top=0, left=0, directories=no, menubar=no, toolbar=no, location=no, resizable=yes, scrollbars=yes, titlebar=yes';
    var TipoArchivo = 'application/pdf';
    var CodTipoArchivo = '5';
...
var PrecioNuevo = Number($(txtPrecio).val());
    try {
        ...
        if (txt.indexOf('txtCantidad') > -1) {
            ...
            hfCodDetalle = txt.replace('txtPrecio', 'hfCodDetalle');
        }
       ...
       var PrecioNuevo = Number($(txtPrecio).val());
         ...
         if (parseInt(Margen)<10 && F_PermisoOpcion(CodigoMenu, 777017, '') === "0")
            ...
            $(Precio).val(Number($(txtPrecio).val()));
            }
...
function F_EliminarRegistro(Fila) {
    try {
        ...
        if ($(lblEstado).text() == "CANCELADO PARCIAL") {
            toastr.warning("ESTA COTIZACION SE ENCUENTRA CANCELADA PARCIAL; PRIMERO ELIMINE LA COBRANZA Y LUEGO ELIMINE LA COTIZACION");
            return false;
        }

        if ($(lblEstado).text() == "CANCELADO TOTAL") {
            toastr.warning("ESTA COTIZACION SE ENCUENTRA CANCELADA TOTAL; PRIMERO ELIMINE LA COBRANZA Y LUEGO ELIMINE LA COTIZACION");
            return false;
        }

        if (!confirm("ESTA SEGURO DE ELIMINAR LA COTIZACION : " + $(lblnumero_grilla).text() + "\nDEL CLIENTE : " + $(lblcliente_grilla).text()))
            return false;
...
function F_ImprimirFacturaHTML(codigo, Fila, rplc, TipoImp, TipoDoc) {
    ...
    { //si es factura ELECTRONICA : impresion ticketera **5** / pdf de frente **4**
        ...
        var CodTipoArchivo2 = 15; // TipoDoc.toString(); //Formato a imprimir FT:1/BO:2/NC:3
        var CodMenu = '218';
        ...
        if (TipoImp == 'TK') {
            ...
            switch (CodTipoArchivo2) {
                case 15: //NOTA DE VENTA
                    ArchivoRpt = "rptFacturaImpresionTicketCT.rpt";
                    break;
...
if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Editar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacio
    try {
        var Cuerpo = '#MainContent_';
        var imgID = "#" + Fila.id;
        var Clave = "";
        ...
        if ($(lblEstado).text() != "PENDIENTE") {
            toastr.warning("LA "+ $("#MainContent_ddlTipoDocConsulta option:selected").text()   + " TIENE PROFORMA O FACTURA");
            return false;
        }
        ...
        F_ObtenerNotaVenta_Net(arg, function (resu) {
            ...
            if (str_mensaje_operacion == "") {
                ...
                $('#hfCodProvincia').val(objprof.CodProvincia);
                $('#hfCodDistrito').val(objprof.CodDistrito);
                $('#hfCodDocumentoRef').val($(hfCodigo).val());
