...
function F_AgregarTemporal(controlId) {
    ...
    if ($(chkSi).is(':checked')) {
        ...
        html += '<input type="hidden" name="ctl00$MainContent$grvDetalleArticulo$ctl02$hfDescripcion" id="MainContent_grvDetalleArticulo_hfDescripcion_' + c + '" value="">';
        ...
        html += '</td>';
        html += '<td align="right" style="fd:none">';
...
function F_ValidarGrabarDocumento() {
    try {
        ...
        if ($(Cuerpo + 'ddlFormaPago') == null | isNaN($(Cuerpo + 'ddlFormaPago').val()) | $(Cuerpo + 'ddlFormaPago').val() == '0' | Number($(Cuerpo + 'ddlFormaPago').val()) < 1)
        ...
        if (($("#MainContent_txtEmpresa").val() == 'CYCLONE PERU S.A.C.' | $("#MainContent_txtEmpresa").val() == 'EVAS 1 E.I.R.L.' | $("#MainContent_txtEmpresa").val() == 'GRUPO CYCLONE MINORISTA E.I.R.L.')) { 
            if ($("#MainContent_ddlSerie option:selected").text()=='0001' & $("#MainContent_ddlSerieGuia option:selected").text()!='0001')
                Cadena = Cadena + '<p></p>' + 'La Factura y la Guia deben tener la misma serie';
        }
        ...
        if ($("#MainContent_txtEmpresa").val() == 'PERNOS TUERCAS & TERCOS S.A.C.'  && $("#MainContent_ddlSerie option:selected").text() == '0002' && $(Cuerpo + 'txtNumero').val() != $(Cuerpo + 'txtNumeroGuia').val())
            Cadena = Cadena + '<p></p>' + 'El numero de la factura debe ser igual al numero de la guia';
        ...
        if ($(Cuerpo + 'txtNroRuc').val().length != 11)
            {Cadena = Cadena + "\n" + "Ruc Invalido";}
            else
            {if (!ValidarRuc($(Cuerpo + 'txtNroRuc').val()) & $(Cuerpo + 'txtNroRuc').val() != '00000000000')
                Cadena = Cadena + "\n" + "Ruc Invalido";}
...
//                    if (vistaprevia == 0) {
                        alertify.log('La factura se grabo correctamente.');
...
if (!F_SesionRedireccionar(AppSession)) return false;
    ...
    {
        if (ValidarRuc($('#MainContent_txtNroRuc').val()) == false & $('#MainContent_txtNroRuc').val() != '0')
        { 
        ...
        else
        {
            ...
            {
                alertify.log('SOLO PERMITIDOS RUC');
                F_LimpiarCampos();
                return false;
            }
            else
                {
                    if ($('#MainContent_txtNroRuc').val() == '0'  | (ValidarRuc($('#MainContent_txtNroRuc').val()) == true & $('#MainContent_txtNroRuc').val().length === 11))
                    {
